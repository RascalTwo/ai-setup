...
def get_token() -> str:
    ...
    for k, v in atl.items():
        if k.startswith(ATLASSIAN_KEY_PREFIX):
            token = v.get("accessToken") or ""
            # An entry can exist with an empty accessToken (OAuth flow registered but
            # never completed, or the server was later removed from the MCP config).
            # Returning "" sends `Authorization: Bearer ` and the server answers 401,
            # which reads like an expired token and sends you chasing the wrong fix.
            if not token:
                raise SystemExit(
                    f"Atlassian MCP entry '{k}' has an EMPTY accessToken "
                    f"(fields: {sorted(v)}).\n"
                    "This is not an expired token — the OAuth flow never completed, and "
                    "there is no refreshToken to renew.\n"
                    "Fix: register the server and authenticate, then retry:\n"
                    "  claude mcp add --transport http atlassian https://mcp.atlassian.com/v1/mcp\n"
                    "  # then run /mcp in Claude Code and authenticate 'atlassian'"
                )
            return token
