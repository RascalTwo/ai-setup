...
rascaltwo-ai-setup:
  ...
  state: false
  integrates:
    github: actions run polling via gh cli
  requires: [gh]
---
...
```bash
.claude/skills/wait-for-github-workflow/scripts/wait-for-github-workflow.sh <run-id-or-latest> [workflow-file] [--job <job-name>]
```

Examples:
- `.claude/skills/wait-for-github-workflow/scripts/wait-for-github-workflow.sh 22379581209`
- `.claude/skills/wait-for-github-workflow/scripts/wait-for-github-workflow.sh latest deploy_bravo.yml`
- `.claude/skills/wait-for-github-workflow/scripts/wait-for-github-workflow.sh latest deploy_bravo.yml --job ldap`
- `.claude/skills/wait-for-github-workflow/scripts/wait-for-github-workflow.sh latest deploy_bravo.yml --job "e2e-test"`
...
If the run/job failed, suggest running `/analyze-github-workflow-failure <run-id>` to investigate.
