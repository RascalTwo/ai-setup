...
`base` is the API (sync server, `localhost:8090` locally); `page` is the web UI
(`localhost:5190` locally — not 5173, which collides with other Vite apps).
...
instead. Use `<page>/llms.txt`. On a deployed instance one host fronts both and it works.

If `page` doesn't answer, the web server is down while the API may still be up.
Restart it detached from `sai-jm-snippets/timeline-studio/web`:
`SYNC_PORT=8090 nohup npx vite preview --port 5190 --strictPort > ~/.timeline-studio/web-preview.log 2>&1 &`
