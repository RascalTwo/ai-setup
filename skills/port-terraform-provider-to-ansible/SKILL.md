...
description: >-
  Port a Terraform provider to a greenfield Ansible collection with full surface parity
  (one module per resource, *_info per data source), hand-built idempotency/check-mode/diff,
  and live acceptance tests. Use when the user wants Ansible modules or a collection for a
  service that has a Terraform provider, says "port the provider", "build an ansible
  collection for X", or "ansible can't manage X natively". Handles partial ports too.
rascaltwo-ai-setup:
  kind: skill
  state: false
  integrates:
    terraform: provider go source read as the spec
    ansible: generated collection modules
