...
**When the context is already a document, pass the PATH, not the contents.** If the
task was written up by `handoff` (or any spec/plan/ADR already on disk), point Codex
at the file:

```bash
codex exec ... "Read <path> and continue the work described there" < /dev/null
```

That keeps the artifact: reviewable before launch, re-runnable after a bad turn,
and handable to a Claude background session instead (`claude --bg --name "<name>"
"Read <path> and continue"`) without rewriting the prompt. Inlining a long summary
buries it in one process's arguments, where nothing can read it again.
