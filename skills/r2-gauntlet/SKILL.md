...
- For `repo`: capture the tracked-file list (`git ls-files`) as the fileset. Note in the
  ...
  reviewing the whole tree, and the auditor family (ln-21, ln-24, ln-25) is in its
  native habitat here. On a `repo` scope, also dispatch `ln-23-test-suite-auditor` — this
...
`ln-12-delivery-reviewer`, `ln-21-documentation-auditor`,
`ln-24-architecture-auditor`, `ln-25-persistence-auditor`,
`ponytail-review`, `code-review`.

Three skills were removed from this roster on 2026-08-30, each disqualified by its own text:
`ln-22-codebase-auditor` ("use when no specialist audit is primary" — never true here, all
...
`ln-23-test-suite-auditor` is **kept but not dispatched**: its own description says "not to
