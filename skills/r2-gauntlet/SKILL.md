...
rascaltwo-ai-setup:
  ...
  integrates:
    claude-code: reviewer subagents and built-in /review and /security-review
---
...
**`$SCRATCH` is `~/.agents/state/r2-sdlc/<repo-id>/gauntlet-<YYYYMMDD-HHMMSS>/`.** Derive
`<repo-id>` exactly as r2-sdlc does — the git remote URL's path portion, `github.com/user/repo`
→ `user--repo`, falling back to a sanitized working-directory basename when there is no remote.
Create the directory before writing to it. This skill shares r2-sdlc's store on purpose (that is
what `state-owner: r2-sdlc` in the manifest records), but a gauntlet run is not a pipeline run,
so it gets its own timestamped directory beside them rather than writing into a `<slug>/`:
...
- For `repo`: capture the tracked-file list (`git ls-files`) as the fileset. Note in the
  ...
  reviewing the whole tree, and the auditor family (ln-53, ln-56, ln-57) is in its
  native habitat here. On a `repo` scope, also dispatch `ln-55-test-suite-auditor` — this
  is the scope where its flake matrix actually runs.
...
- **fidelity-reviewer** needs a design doc (oracle = "does impl match the design?"). Look for
  ...
  `~/.agents/state/r2-sdlc/<repo-id>/*/design.md` (most recent). If exactly one obvious
...
`ln-52-delivery-reviewer`, `ln-53-documentation-auditor`,
`ln-56-architecture-auditor`, `ln-57-persistence-auditor`,
`ponytail-review`, `code-review`.

Three skills were removed from this roster on 2026-08-30, each disqualified by its own text:
`ln-54-codebase-auditor` ("use when no specialist audit is primary" — never true here, all
specialists always run) and `ponytail-audit` ("ponytail-review, repo-wide" — nothing unique,
and it was handed a diff while instructed to scan the tree) are **deleted**.
`ln-55-test-suite-auditor` is **kept but not dispatched**: its own description says "not to
review one delivery", and on a diff it cannot run its flake matrix — the one thing it does
that `test-reviewer` cannot. Invoke it standalone when chasing a flake.
...
## 🔴 Blockers
### [src/foo.ts:12-40] <title>  ·  flagged by code-reviewer, ponytail-review, ln-56 (consensus ×3)  ·  over-engineering
<detail> — **Fix:** <fix>
...
deliberate `◇` skip here: `ln-56-architecture-auditor` already covers that dimension for a
change review.
