...
3. **Persist everything.** All phase artifacts go to `~/.agents/state/r2-sdlc/<repo-id>/<slug>/`. Never inside the repo. Never committed.
...
3. **Check for existing pipeline run.**
   - Does `~/.agents/state/r2-sdlc/<repo-id>/<slug>/` already exist?
...
3. **Decide per-finding:**
   ...
   - Blockers → fix in code, then **re-run only the touched reviewers** via the gauntlet's
     ...
     change (e.g. edited test files → `--only test-reviewer,ln-55-test-suite-auditor`). Cap at
...
1. **Produce the summary** for the user, containing:
   ...
   - Link to the pipeline dir (`~/.agents/state/r2-sdlc/<repo-id>/<slug>/`).
...
- Create pipeline artifacts inside the repo. They live at `~/.agents/state/r2-sdlc/`.
