...
echo
echo "## Error signal"
# Count signatures ONLY on tool-result lines that are ACTUALLY ERRORS.
# Two rounds of this bug now:
#   1. Scanning the whole transcript counted lines that merely *discuss* an error
#      (prose, pasted logs, this report's own past output) -- inflated ~7x.
#   2. Restricting to tool-result lines was NOT enough: a SUCCESSFUL tool_result still
#      matches if it merely CONTAINS the string. The aws-sso-creds / fmh-aws /
#      sourceallies-aws skill descriptions literally enumerate "ExpiredToken,
#      InvalidClientTokenId, UnrecognizedClientException, or Unable to locate
#      credentials" as their trigger conditions, so every load scored 4 phantom errors.
#      The 2026-09-19 audit chased "287 AWS credential errors"; the real count was 1.
# So: the line must ALSO carry "is_error":true before its signatures are counted.
# Falsifier for anyone re-touching this: a session that merely READS a skill whose text
# names these signatures must add ZERO to the counts below.
ERR=$(grep -h '"is_error":true' "$TMP" 2>/dev/null)
printf '%s\n' "$ERR" | grep -c . | sed 's/$/ errored tool_results (is_error:true)/'
printf '%s\n' "$ERR" \
  | grep -hoE 'NOOP_EDIT|TEXT_NOT_FOUND|FILE_NOT_FOUND|String not found|out of range|hash mismatch|permission denied|tool_use_error|InputValidationError|ExpiredToken|Unable to locate credentials' \
  | sort | uniq -c | sort -rn
