...
rascaltwo-ai-setup:
  ...
  integrates:
    crv: frame selection and on-screen-text index
    mlx-vlm: local video model
  requires: [ffmpeg, mlx-vlm]
---
...
`~/.agents/state/read-video-locally/cache/crv`, keyed by path+mtime+size, so a second question
about the same video is fast.
...
- **Privacy:** crv indexes on-screen text of everything it watches into a memory db — a
  ...
  start a corpus. Pass `--remember` to opt in, and what it feeds is crv's own corpus at
  `~/.crv/memory.db` — where bare `crv` writes and `crv-ask` already reads, no env var needed.
  That db is **crv's, not this skill's**, which is why it does not live under
  `~/.agents/state/`: redirecting it would split the corpus in two and leave `crv-ask` searching
  the half this skill never wrote.
