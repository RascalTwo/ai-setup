...
STATE_DIR = os.path.expanduser("~/.agents/state/read-video-locally")
CACHE_DIR = os.path.join(STATE_DIR, "cache")   # crv frame output, keyed by path+mtime+size
# crv's OWN corpus, not this skill's state. Any bare `crv` run writes it and
# `crv-ask` searches it, so redirecting it here would split the user's index in
# two and leave crv-ask finding nothing. Same line drawn for ~/.voicemode.
MEMORY_DB = os.path.expanduser("~/.crv/memory.db")  # crv owns this; we only add to it with --remember
...
def run_crv(video: str) -> str:
    ...
    CRV_NO_MEMORY=1: crv otherwise indexes on-screen text of everything it sees into its memory db.
    ...
    With --remember this contributes to crv's own index at ~/.crv/memory.db, which is
    where `crv-ask` already looks. Nothing is redirected.
    """
    key = hashlib.sha1(f"{os.path.realpath(video)}:{os.path.getmtime(video)}:"
                       f"{os.path.getsize(video)}".encode()).hexdigest()[:16]
    out = os.path.join(CACHE_DIR, "crv", key)
...
def main() -> None:
    ...
    ap.add_argument("--remember", action="store_true",
                    help="let crv index this video's on-screen text into its own corpus at "
                         "~/.crv/memory.db, where `crv-ask` already looks. Off by default: this "
                         "skill gets pointed at client material and shouldn't quietly start a corpus.")
