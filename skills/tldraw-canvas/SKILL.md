...
rascaltwo-ai-setup:
  ...
  state: false
  integrates:
    tldraw: sync server agent API and tldraw.com boards
    chrome: browser transport via Chrome MCP
---
...
| Verification | authoritative read — **plus a screenshot for layout** | screenshot |
...
4. **Background furniture** — gridlines, bands, axis rules, backdrops — is bounded
   by the content region it annotates, **never by the layout's total extent**. It
   underlaps content by design, so the overlap check in (2) reads clean and tells
   you nothing about it. Get this wrong and a chart's deadline line runs straight
   down through the legend prose sitting below the chart.
...
**Derive those ids deterministically** — from position, or from the source record
they represent — rather than from a counter that restarts each run. Then a board
generated by a script can be re-rendered as an overwrite in place, and the delete
sweep fires only on shapes that genuinely went away. A rebuild that shifts every
shape on the page still comes back `put 409, delete 0`.
...
**Records are the oracle for what exists. Pixels are the oracle for how it lays
out.** Don't collapse the two.

For presence and sync state, prefer records wherever the transport offers them: on
HTTP, `GET /agent/room/:roomId` is the source of truth, and **a browser tab is
never that oracle** — a hidden tab defers applying incoming sync messages, so it
will report a shape missing long after the write landed and every visible viewer
already has it.

For layout, an authoritative read proves nothing. It echoes back the `x`/`y`/`w`
you asked for, so a label that overflowed its box and collided with the shape below
reads as perfectly clean. **Anything text-heavy gets a screenshot too** — including
over HTTP, where nothing measured the text in the first place. The transport files
carry the recipe.
