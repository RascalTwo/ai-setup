...
All enums empirically derived from live validator responses. If tldraw ships a new version with new colors/shapes, the validator will still tell you exactly what's valid — `POST` a record with an intentionally-bad value and read the `400` message, which lists every accepted option.
...
`references/records.md` has worked templates for `geo`, `note`, `text`, `frame`,
`arrow` (+ bindings), and this deployment's custom `logic-gate`. The others
(`draw`, `line`, `bookmark`, `image`, `video`, `embed`, `highlight`) have no
template — read a real one out of an existing room with
`GET /agent/room/<roomId>?full=1` and copy its prop shape.
