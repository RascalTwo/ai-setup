...
description: >-
  Port a Terraform provider's resource surface and acceptance-test behaviors to a Kubernetes
  operator (Java/Quarkus + JOSDK by default) with measured parity — CRDs with full lifecycle,
  an e2e harness against the real API, and an effort ledger. Use when the user wants a K8s
  operator for a service that has a Terraform provider, asks for "CRDs for <service>", or
  wants "operator parity with the <X> provider". Handles resuming a partial port.
rascaltwo-ai-setup:
  kind: skill
  state: false
  integrates:
    terraform: provider surface read as the spec
    kubernetes: generated operator and CRDs
    github: gh cli
  requires: [gh, jq, java, maven, kubectl, kind, docker]
---
...
2. **Target API + auth**: test against a **LOCAL dockerized instance of the
   service, and only that** (the provider's own CI shows how to run one) —
   reproducible on anyone's machine, faster, version-matrixable. The harness
   spins its own container (`up` script) and bootstraps its own
   service-account credential; write both as part of the harness. Do NOT
   build dual-path logic or flags for remote instances: the target is just
   URL+creds env vars, so pointing elsewhere is already a zero-code override —
   leave it at that.
