...
# (use Read tool with offset/limit on ~/.agents/state/ping-api/spec-cache/pingone-platform.yaml)
```

The spec is cached at `~/.agents/state/ping-api/spec-cache/pingone-platform.yaml` and auto-refreshed after 7 days.
...
The script obtains a bearer token via client_credentials grant and caches it at `~/.agents/state/ping-api/token-cache.json`. Tokens are reused until they expire (typically 1 hour). Delete the cache file to force a fresh token.
