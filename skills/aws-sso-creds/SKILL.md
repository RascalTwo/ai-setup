...
rascaltwo-ai-setup:
  ...
  requires: [aws, chrome, claude-in-chrome]
---
...
1. Credentials written to `~/.agents/state/aws-sso-creds/<portal>-<account>.env`, mode `600`.
...
```bash
set -a && source ~/.agents/state/aws-sso-creds/<portal>-<account>.env && set +a && aws sts get-caller-identity
```
...
(() => {
  ...
  for (const d of dlgs) {
    // Account names contain spaces ("Example Corp Dev"), so match anything up to
    // the "(<id>)" rather than a single non-space token — \S+? silently fails to
    // match every multi-word account and reports NO_CRED_DIALOG on a valid modal.
    const m = d.textContent.match(/Create access for the account\s+(.+?)\s*\((\d+)\)/);
...
**`NO_CRED_DIALOG` means "no modal found", not "wrong account" — never treat it as
a pass.** If you get it while a credentials modal is clearly open, the selector or
the regex has drifted from the portal's markup; confirm the account visually from a
screenshot before copying anything, and fix the snippet.
...
OUT="$HOME/.agents/state/aws-sso-creds/${PORTAL}-${ACCOUNT}.env"

mkdir -p ~/.agents/state/aws-sso-creds && chmod 700 ~/.agents/state/aws-sso-creds
umask 077
...
`set -a && source ~/.agents/state/aws-sso-creds/<portal>-<account>.env && set +a && ...`,
...
- Never commit `~/.agents/state/aws-sso-creds/*.env` or include them in diffs.
