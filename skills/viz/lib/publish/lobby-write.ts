...
export type Card = { slug: string; title: string; description: string; tags: string[]; kind: string; private: boolean; spoiler: boolean; image?: string; mtime: number; created: number };
...
export async function writeLobby(
  ...
  // Only for a real dist build. On an IN-PLACE mirror rebuild outRoot === srcContainer,
  // and writing here would drop a build artifact into somebody's source tree (it did,
  // into sai-project-hydra/viz-pages/, before this guard).
  if (path.resolve(outRoot) !== path.resolve(srcContainer)) {
    await Bun.write(path.join(outRoot, "404.html"), render404(pageTitle, haveHost ? shareHost : ""));
  }
}

// A viz's id IS its URL (ADR 0001), so `viz move` necessarily 404s the old address — there
// is no redirect layer and deliberately so. Moving 39 vizzes between containers in one pass
// (2026-09-18) turned that from a footnote into 39 dead links, most of them shared somewhere.
// This is the cheap half of the fix: a static host serves /404.html for any unmatched path
// (GitHub Pages and GitLab Pages both do, by filename convention — no config), so a stale
// link lands somewhere that explains itself and offers the index instead of a bare 404.
//
// It deliberately says nothing about WHAT was there or WHERE it went: on a sealed container
// this page is served unencrypted, so anything specific would leak past the lobby password.
function render404(siteTitle: string, shareHost: string): string {
  // Absolute, not "/": a Pages project site lives at /<repo>/, and 404.html is served
  // for unmatched paths at ANY depth, so a relative href would resolve differently
  // per URL. Fall back to "./" only when there is no host (a local --out build).
  const home = shareHost ? shareHost.replace(/\/?$/, "/") : "./";
  const t = siteTitle.replace(/[&<>"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[c]!);
  return `<!doctype html>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Not here — ${t}</title>
<style>
  ...
  p { margin:0 0 1.25rem; color:#9aa7b2 }
  a { display:inline-block; padding:.55rem 1.1rem; border-radius:7px;
      background:#1f6feb; color:#fff; text-decoration:none; font-weight:550 }
  a:hover { background:#388bfd }
</style>
<main>
  <h1>Nothing at this address</h1>
  <p>This page may have moved to another collection, been renamed, or never existed.
     The index below lists everything that is here now.</p>
  <a href="${home}">Browse ${t}</a>
</main>
`;
}
