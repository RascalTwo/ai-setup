...
import { guardLinks } from "../library/linked.ts";
import { nativeSlugsOf } from "../library/viz.ts";
...
export async function rotateKey(arg: string, o: PublishOptions = {}): Promise<void> {
  ...
  const positional = ["", arg];
    ...
    // A new key kills the old magic links: the viz's own, or with --lobby every lobby-sealed
    // viz in the container. Any of them being linked refuses (ADR 0016).
    guardLinks(lobbyFlag ? [...nativeSlugsOf(abs)].map((s) => path.join(abs, s)) : [abs],
      `rotate the ${lobbyFlag ? "lobby key" : "key"} of ${base}`, o.breakLinks === true);
    if (lobbyFlag) {
...
export async function previewContainer(arg: string, o: PublishOptions = {}): Promise<void> {
  ...
  const positional = ["", arg];
    ...
    const { server, shareHost, buildDone } = servePreview(container, previewRoot, baseUrl, { noIndex, indexTitle, indexDescription, gateApproval: false }, port);
    const summary = await buildPublishableTree(container, previewRoot, shareHost, { noIndex, indexTitle, indexDescription, gateApproval: false });
