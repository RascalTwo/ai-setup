...
export function renderLobby(
  vizzes: { slug: string; title: string; description: string; tags: string[]; scaffold: string; private: boolean; spoiler: boolean; thumb?: string; mtime: number; created: number }[],
  pageTitle: string,
  preamble: string,
  ...
  const cards = vizzes
    .map((v) =>
      v.private
        ...
        : `      <a class="card${v.thumb ? " has-thumb" : ""}${v.spoiler ? " has-spoiler" : ""}"${v.spoiler ? ` data-spoiler="1"` : ""} data-search="${escHtml([v.title, v.slug, v.description, ...v.tags, v.scaffold].join(" ").toLowerCase())}" data-tags="${escHtml(v.tags.join("|"))}" data-scaffold="${escHtml(v.scaffold || "page")}" data-posture="public" data-slug="${escHtml(v.slug)}" data-mtime="${v.mtime}" data-created="${v.created}" href="./${escHtml(v.slug)}/">\n` +
          ...
          `        <div class="card__body">\n` +
          (v.tags.length || v.scaffold
            ? `          <div class="tags">\n` +
              (v.scaffold ? `            <span class="tag tag--scaffold">${escHtml(v.scaffold.replace("-", " "))}</span>\n` : "") +
...
<style>
  ...
  *{box-sizing:border-box}html,body{margin:0;height:100%}
  ...
  .card .tag--scaffold{color:var(--muted);border:1px solid currentColor;border-radius:999px;padding:1px 7px;letter-spacing:.06em}
...
${cards}
  ...
  (function(){
    ...
    var groups=[
      ...
      {key:"Scaffold",attr:"scaffold",label:function(v){return cap(v.replace("-"," "));},
        values:uniq(cards.map(function(c){return c.dataset.scaffold;}))}
    ];
