...
// `scaffold` is the pre-template name of `kind`, still present in sidecars written before the
// rename; readers take `kind ?? scaffold`.
export type SidecarCard = { title: string; description: string; tags: string[]; kind: string; scaffold?: string; listed: boolean; private: boolean };
...
export function resolveMirrorCard(vizDir: string, entry: MirrorVizEntry): SidecarCard {
  ...
  return {
    ...
    kind: base.kind,   // structural fact of the page — never overridable
...
export function composeCards(sourceContainer: string): { cards: Card[]; unlisted: number } {
  ...
  for (const dir of vizzesIn(sourceContainer)) {
    ...
    if (side) {
      ...
      const { title, description, tags, kind, scaffold, private: isPriv } = side.card;
      ...
      cards.push({ slug, title, description, tags, kind: kind ?? scaffold ?? "", private: isPriv, spoiler: false, image: isPriv ? undefined : findOgImage(dir) || undefined, mtime: vizMtime(dir), created: vizCreated(dir) });
    } else {
