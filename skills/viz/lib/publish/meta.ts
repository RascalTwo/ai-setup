...
// starter's <!-- <meta name="viz:spoiler" …> --> example isn't parsed as a real declaration.
...
// A page's KIND (deck, poster, exchange…). A template is a page of the kind it is a
// template for — checked FIRST, because a copy re-marked as a template of a new kind still
// carries the viz:page-kind it was born with. A copy made from a template carries
// viz:page-kind; pages scaffolded before templates existed carry the legacy viz:scaffold. The one reader
// of all three names, so nothing else has to know the history. "" = a plain page.
//
// NOT viz:kind: that name belonged to the explanatory/operational axis ADR 0011
// removed, and pages that were never swept still carry it with those values.
export function kindOf(html: string): string {
  return (grabMeta(html, "viz:template") || grabMeta(html, "viz:page-kind") || grabMeta(html, "viz:scaffold")).toLowerCase();
}

export function vizCardMeta(html: string): { title: string; description: string; tags: string[]; kind: string } {
  return {
    ...
    // Absent on a plain page; the lobby materialises that absence as "page" so the
    // facet can offer it as a choice (see renderLobby).
    kind: kindOf(html),
  };
}
...
// The viz's durable identity (see lib/create/uid.ts). Random, minted once, carried in
// index.html so a move preserves it. Returns "" for a viz created before ADR 0014 that
// has not been backfilled — callers that need durable identity must handle that.
export function readUid(vizDir: string): string {
  const indexPath = path.join(vizDir, "index.html");
  if (!existsSync(indexPath)) return "";
  return grabMeta(readFileSync(indexPath, "utf8"), "viz:uid").trim();
}
