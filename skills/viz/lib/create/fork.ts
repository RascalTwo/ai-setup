// lib/create/fork.ts — `--from`: copying an existing viz (usually a template) as the
// starting point.
//
// A fork ALWAYS resets to local/unlisted — posture is a trust decision and is never
// inherited — and drops the source's comments, og.auto.png and recordings. It keeps
// the source's KIND but not its template-ness: a deck made from a deck template is a
// deck, not another template.
//
// Extracted from bootstrap.ts.

import { mintUid } from "./uid.ts";
...
import { kindOf } from "../publish/meta.ts";
...
// Delete every <meta name=NAME>, and the <!-- NAME … --> comment that explains it, so a
// copy never carries a declaration that was only true of its source.
export function removeMeta(html: string, name: string): string {
  return html
    .replace(new RegExp(`[ \\t]*<!--\\s*${name}\\b[\\s\\S]*?-->[ \\t]*\\n?`, "gi"), "")
    .replace(new RegExp(`[ \\t]*<meta\\s+name=["']${name}["'][^>]*>[ \\t]*\\n?`, "gi"), "");
}
...
export function forkFrom(srcDir: string, destDir: string, newSlug: string): void {
  ...
  html = setMeta(html, "viz:uid", mintUid()); // a fork is a NEW viz — never share an identity
  ...
  html = html.replace(/<title>[\s\S]*?<\/title>/i, `<title>${newSlug}</title>`);
  // Kind carries over; template-ness does not. viz:scaffold is the pre-template name
  // of viz:page-kind, so a fork of an older page is normalised to the current one.
  const kind = kindOf(html);
  for (const name of ["viz:template", "viz:scaffold", "viz:page-kind"]) html = removeMeta(html, name);
  if (kind) html = setMeta(html, "viz:page-kind", kind);
  // Links record who points at the SOURCE's URL (ADR 0016). The copy has a new URL.
  html = removeMeta(html, "viz:linked-from");
