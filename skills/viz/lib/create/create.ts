...
export async function createViz(opts: CreateOptions): Promise<void> {
  ...
  // Stamp the durable identity AFTER the scaffold is written, so all four scaffolds (and
  // a fork, which mints its own in forkFrom) get one without threading a uid parameter
  // through every template signature. See lib/create/uid.ts for why it is random.
  {
