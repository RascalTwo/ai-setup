// lib/create/create.ts — creating a new viz: the blank starter, or a copy of a template.
//
...
import { starterHtml, heroHtml } from "./templates.ts";
import { resolveTemplate, looksLikePath } from "../library/templates.ts";
import { grabMeta } from "../publish/meta.ts";
import { mintUid, setUidMeta } from "./uid.ts";
import { forkFrom } from "./fork.ts";

// This module lives two levels down from the skill root; the vendored runtime source
// is relative to that root, not here.
const SKILL_DIR = path.resolve(import.meta.dir, "../..");
const VIZ_ROOT = CENTRAL;
const DUMP_LIMIT = 8 * 1024;
...
export type CreateOptions = {
  ...
  localDir?: string;
  hero?: boolean;
  /** A template name (see `viz templates`) or a path to any viz folder. */
  from?: string;
  runtime?: boolean;
  quick?: boolean;
  print?: boolean;
...
export async function createViz(opts: CreateOptions): Promise<void> {
  const {
    slug, local = false, localDir, hero = false, from, runtime = false, quick = false,
  ...
  if (from) {
    const srcDir = looksLikePath(from) ? path.resolve(expandHome(from)) : await resolveTemplate(from, path.resolve(from));
    ...
    if (!jsonMode) {
      // stdout must stay parseable under --json — the MCP tool reads it
  ...
  // Stamp the durable identity AFTER the page is written, so the starter (and a fork,
  // which mints its own in forkFrom) gets one without threading a uid parameter through
  // the template. See lib/create/uid.ts for why it is random.
  {
    const idx = path.join(slugDir, "index.html");
    const html = readFileSync(idx, "utf8");
    if (!/<meta\s+name=["']viz:uid["']/i.test(html)) writeFileSync(idx, setUidMeta(html, mintUid()));
  }

  // --hero writes a starter hero.html beside it. Meaningless for a page that is already
  // its own card (viz:card=self, e.g. a poster) — say so rather than quietly writing a
  // file that would be ignored (verify.ts --og prefers hero.html, so it would actually
  // shadow the page's own card).
  const isOwnCard = grabMeta(readFileSync(path.join(slugDir, "index.html"), "utf8"), "viz:card") === "self";
  if (hero) {
    if (isOwnCard) {
      if (!jsonMode) console.log(`Note:    --hero ignored — this page IS its own OG card (viz:card=self).`);
    } else {
  ...
  if (local) {
    ...
    console.log(`Edit:    index.html is ALREADY written (posture metas pre-stamped) — edit it directly; keep the viz:* metas.`);
    console.log(
      runtime
  ...
  // Dump the page so the agent can edit it straight away. Printing the bytes here
  // costs the same as reading them and saves the round trip; it also means a viz
  // created under a non-Claude-Code agent (no read-before-write guard at all) still
  // gets the contract — the posture metas — in front of the author.
  // A big page (a deck is ~15KB, a brand template far more) is not dumped: the agent
  // rarely needs it verbatim, and it would crowd out everything else. Read it instead.
  ...
  } else if (print && bytes <= DUMP_LIMIT) {
    console.log(`\n${"─".repeat(70)}\nindex.html (already written — edit it, keep the viz:* metas)\n${"─".repeat(70)}`);
    console.log(body);
    console.log("─".repeat(70));
    if (hero && !isOwnCard) console.log(`hero.html also written — see it with: cat "${path.join(slugDir, "hero.html")}"`);
  } else if (print) {
    console.log(`Print:   index.html not dumped (${Math.round(bytes / 1024)}KB) — read it before editing; its comments are the authoring guide.`);
  }
