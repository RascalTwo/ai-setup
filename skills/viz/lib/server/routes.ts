...
import { rebuild, resolve, runScan, setStatus, slugMap } from "./state.ts";
...
export async function handleRequest(req: Request, server: Server): Promise<Response | undefined> {
    ...
    // Build status. POST "ready" at handoff; "building" is inferred from writes, so
    // this is the only signal the agent has to send by hand. Body is the state string.
    if (rest === "_status") {
      if (req.method !== "POST") return new Response("POST only", { status: 405 });
      setStatus(entry.id, (await req.text()).trim().slice(0, 40));
