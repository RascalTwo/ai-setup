 ...
 *  several have it — never guesses. `bareDir`, if it exists, is used when no template has
 *  the name, so a bare folder name in the cwd still works as it did before templates. */
export function resolveTemplate(name: string, bareDir?: string): string {
  const hits = listTemplates().filter((t) => t.name === name);
...
/** `viz templates [kind]`: human listing, or --json. */
export function cmdTemplates(kind: string | undefined, flags: Record<string, string | boolean>): void {
