...
describe("templates replace scaffolds", () => {
  ...
  const templates = async (args: string[] = []) =>
    JSON.parse((await cli("viz.ts", ["templates", ...args, "--json"])).stdout) as { name: string; kind: string; dir: string; duplicate: boolean }[];
  const head = (slug: string) => Bun.file(path.join(LIB, slug, "index.html")).text();
  ...
  test("Given --from with --json, when create runs, then stdout is only the JSON", async () => {
    ...
    expect(r.code).toBe(0);
    expect(JSON.parse(r.stdout).slug).toBe("from-json");
  });
  ...
  test("Given a copy re-marked as a template of another kind, when copied, then the copy takes the template's kind", async () => {
    ...
    await Bun.write(idx, (await Bun.file(idx).text()).replace("</head>", '<meta name="viz:template" content="brand-deck">\n</head>'));
    const r = await cli("viz.ts", ["create", "from-brand", "--from", path.join(LIB, "from-deck"), "--no-print"]);
    expect(r.code).toBe(0);
    expect(await head("from-brand")).toContain('<meta name="viz:page-kind" content="brand-deck">');
  });

  test("Given --hero on a page that is its own card, with --json, then stdout is still only the JSON", async () => {
    const r = await cli("viz.ts", ["create", "hero-poster", "--from", "poster", "--hero", "--json"]);
    expect(r.code).toBe(0);
    expect(JSON.parse(r.stdout).slug).toBe("hero-poster");
  ...
  test("Given the exchange template, when copied, then its content.js comes along", () => {
    expect(existsSync(path.join(LIB, "from-exchange", "content.js"))).toBe(true);
  });

  test("Given a path to a template, when create --from runs, then paths still work", async () => {
    const r = await cli("viz.ts", ["create", "from-path", "--from", path.join(SKILL, "viz-pages", "deck"), "--no-print"]);
    expect(r.code).toBe(0);
  ...
  test("Given a name no template has, when create --from runs, then it fails pointing at `viz templates`", async () => {
    const r = await cli("viz.ts", ["create", "from-nothing", "--from", "no-such-template", "--no-print"]);
    expect(r.code).not.toBe(0);
  ...
  const plantTemplate = (repo: string, name: string) => {
    ...
    writeFileSync(path.join(dir, "index.html"), '<html><head><title>t</title><meta name="viz:template" content="scan-kind"></head></html>');
  };
  ...
  test("Given an unscanned template, when create --from names it, then it is found without a separate rescan", async () => {
    ...
    const r = await cli("viz.ts", ["create", "from-unscanned", "--from", "scan-probe-2", "--no-print"]);
    expect(r.code).toBe(0);
    expect(await head("from-unscanned")).toContain('<meta name="viz:page-kind" content="scan-kind">');
  });
  ...
  test("Given two templates with the same name, when listed and copied by name, then it is flagged and refused", async () => {
    ...
    writeFileSync(path.join(LIB, "deck", "index.html"), '<html><head><meta name="viz:template" content="deck"></head></html>');
    try {
...
describe("listing reads the metas pages actually write", () => {
  test("Given a viz with repeated viz:tag metas, when ls --json runs, then its tags are listed", async () => {
    ...
    const rows = JSON.parse((await cli("viz.ts", ["ls", "--json"])).stdout) as { id: string; tags: string }[];
...
describe("linked vizzes refuse to break their URL (ADR 0016)", () => {
  ...
  const entries = async () => {
    const rows = JSON.parse((await cli("viz.ts", ["ls", "--json"])).stdout);
    return rows.find((r: any) => r.dir === dir()).linkedFrom as string[];
  };

  test("Given a viz, when --linked-from is repeated, then each entry is added verbatim (commas kept)", async () => {
    await cli("bootstrap.ts", ["linked-probe", "--no-print"]);
    const r = await cli("viz.ts", ["update", dir(), "--linked-from", "QR Codes, Confluence page", "--linked-from", "https://x.slack.com/a?b=1&c=2", "--no-commit"]);
    expect(r.code).toBe(0);
    expect((await entries()).sort()).toEqual(["QR Codes, Confluence page", "https://x.slack.com/a?b=1&c=2"].sort());
  });
  ...
  test("Given a linked viz, when it is copied with --from, then the copy is not linked", async () => {
    ...
    expect(r.code).toBe(0);
    expect(await Bun.file(path.join(LIB, "linked-copy", "index.html")).text()).not.toContain("viz:linked-from");
  });

  test("Given an existing entry, when it is added again, then nothing changes", async () => {
    const r = await cli("viz.ts", ["update", dir(), "--linked-from", "QR Codes, Confluence page", "--no-commit"]);
    expect(r.code).toBe(0);
  ...
  test("Given a linked viz, when the self-portrait's manage.ts path moves it, then it refuses too", async () => {
    const r = await cli("manage.ts", ["move", dir(), path.join(LIB, "linked-moved"), "--no-commit"]);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain("--break-links");
  });

  test("Given the self-portrait's manage.ts path, when it adds and removes an entry, then both land", async () => {
    expect((await cli("manage.ts", ["update", dir(), "--linked-from", "via manage", "--no-commit"])).code).toBe(0);
    expect(await entries()).toContain("via manage");
    expect((await cli("manage.ts", ["update", dir(), "--unlinked-from", "via manage", "--no-commit"])).code).toBe(0);
    expect(await entries()).not.toContain("via manage");
  });

  test("Given a linked viz, when title/tags/listed change, then nothing is guarded", async () => {
    const r = await cli("viz.ts", ["update", dir(), "--title", "T", "--tags", "a", "--listed", "listed", "--no-commit"]);
    expect(r.code).toBe(0);
  });
  ...
  test("Given --break-links, when each guarded operation runs, then it goes ahead", async () => {
    for (const argv of [
      ...
      const r = await cli("viz.ts", argv);
      expect({ argv, code: r.code, err: r.stderr }).toEqual({ argv, code: 0, err: expect.any(String) });
    }
  ...
  test("Given the last entry removed, when the viz moves, then it is no longer guarded", async () => {
    ...
    expect((await cli("viz.ts", ["update", dir(), "--unlinked-from", "only", "--no-commit"])).code).toBe(0);
