...
if (!target) {
  console.error("usage: bun verify.ts <url|id> [--wait=<selector|ms>] [--full] [--size=WxH] [--og] [--commit=<msg>]");
  process.exit(2);
}
...
let layout: { findings: string[]; census: string; words: number } = { findings: [], census: "?", words: 0 };
...
try {
  ...
  const auditPage = () => page.evaluate(() => {
    ...
    return { findings: out, census: census || "nothing", words };
  }).catch((e) => ({ findings: [`layout audit failed: ${(e as Error).message}`], census: "?", words: 0 }));
