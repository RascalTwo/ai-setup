...
// the central library (~/.agents/state/viz, or an older path — see resolveVizRoot)
// OR inside any repo,
...
//   2. ~/.agents/state/viz   — the repo's state convention, and the default for fresh installs
//   3. ~/.viz-pages          — former default, kept so existing libraries aren't orphaned
//   4. ~/.claude/viz-pages   — original location, kept for the same reason
// Both older paths stay as fallbacks: this skill is distributed, and a machine whose
// library has not been moved yet must keep working.
export function resolveVizRoot(): string {
  if (process.env.VIZ_PAGES_DIR) return process.env.VIZ_PAGES_DIR;
  const preferred = path.join(HOME, ".agents", "state", "viz");
  for (const dir of [preferred, path.join(HOME, ".viz-pages"), path.join(HOME, ".claude", "viz-pages")]) {
  ...
  for (const container of containers) {
    ...
    try {
      names = readdirSync(container, { withFileTypes: true })
        // "_"-prefixed dirs are build infra (_thumbs), never vizzes — same intent as "."
