...
const VALUE_FLAGS = new Set(["posture", "listed", "approved", "to", "access", "title", "description", "tags", "n", "linked-from", "unlinked-from"]);
...
const USAGE =
  ...
  "  bun manage.ts move   <viz-folder> <dest-folder> [--break-links]\n" +
  "  bun manage.ts delete <viz-folder> [--break-links]\n" +
  "  bun manage.ts update <viz-folder> [--posture …] [--listed …] [--approved …] [--title …] [--description …] [--tags a,b,c] [--linked-from <text>] [--unlinked-from <text>] [--break-links]\n" +
...
if (verb === "ls") {
  ...
  maybeCommit(cmdMove(resolveViz(pos[0]), pos[1], flags["break-links"] === true), noCommit, `viz: move ${path.basename(path.resolve(pos[0]))} → ${path.basename(path.resolve(pos[1]))}`);
} else if (verb === "delete") {
  const viz = resolveViz(pos[0]);
  maybeCommit(cmdDelete(viz, flags["break-links"] === true), noCommit, `viz: delete ${viz.slug}`);
} else if (verb === "update") {
  const viz = resolveViz(pos[0]);
