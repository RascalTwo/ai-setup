...
import { idFor, buildSlugMap, HOME } from "./discovery.ts";
...
type RawTarget = { path: string; vizzes: any[] };
type RawMirrors = { mirrors: RawTarget[]; vendors?: RawTarget[] };
type EdgeKind = "mirrors" | "vendors";
...
function tidyRaw(raw: RawMirrors): RawMirrors {
...
function printRows(rows: Row[], json: boolean): void {
  if (json) {
...
function cmdLs(flags: Record<string, string | boolean>): void {
  let rows = corpus();
  for (const axis of ["posture", "listed", "kind"] as const) {
...
function cmdSearch(term: string, flags: Record<string, string | boolean>): void {
...
function cmdVendor(viz: Viz, sinkInput: string, flags: Record<string, string | boolean>): string[] {
...
function cmdVendorRm(viz: Viz, sinkInput: string): string[] {
  const sinkContainer = path.resolve(sinkInput);
  const file = path.join(viz.container, "mirrors.json");
  const raw = loadMirrorsRaw(file);
...
function cmdVendorLs(viz: Viz): null {
...
const USAGE =
  "usage:\n" +
  "  bun manage.ts ls     [--posture …] [--listed …] [--kind …] [--central|--local] [--json]\n" +
