...
- [2026-08-06] **PROMOTED — deck machinery.** `kit/deck.css` + `kit/deck.js`, and
  ...
  [2026-09-24] **Rebuilt on reveal.js 5.2.1** for speaker view + PDF export, keeping
  the same DOM contract, `go()` and `deck:slide`; reveal.css is NOT loaded (it
  would restyle every slide). The cascade class was renamed `.reveal` →
  `.cascade` because `.reveal` is reveal's root. See reference/cdn.md.
...
- `stepper({autoplayMs})` conflates "autoplay is available" with "autoplay starts now"
  (`if (autoplayMs) play()` at construction). A viz that wants a working Play button but no
  motion on arrival has to call `.pause()` immediately after constructing. An `autostart:false`
  option, or having `play()` accept an interval, would remove the workaround.
  Seen in: sai-project-hydra-team-marketing/viz-pages/sprint-rollover (2026-09-18).
