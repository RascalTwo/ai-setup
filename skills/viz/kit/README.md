...
## Pointing at something on screen? See `ui-narration` first

If a viz needs a cursor that glides to a target, a highlight box that frames an element, or a
click pulse, **do not re-derive the timings** — the `ui-narration` skill has them
(`glide: 950, hold: 550`, and a halo that matches the target's computed border radius). It is a
plain IIFE exposing `window.__narrate`; drop it in and drive it. Nothing in this kit duplicates it,
and nothing should.
...
swatch — colour it with `background`, `.dot` for a round one), `.pill` (uppercase status badge; `.pill.good` /
`.warn` / `.danger` / `.accent` tint text + border together), `.flow` (animated
dashed edge), `.vsvg-label` (used by `labelBox()`).

**Naked `<a>` is styled now** — `--accent`, `--c4` when visited, `--text` on hover.
Until 2026-08-19 there was no bare `a` rule, so every anchor in every viz fell
through to the user-agent default: `#0000EE` on `#0d1117` (2.3:1) and `#551A8B`
once visited (1.5:1 — effectively invisible). Both tiers are tokens, so they
follow a re-theme, and the rule is at the lowest possible specificity, so any
component rule still wins. Verified empirically that `var()` **does** resolve
inside `:visited` (Chrome permits `color` there).
...
`viz create <slug> --hero` scaffolds a `hero.html` that already does this —
