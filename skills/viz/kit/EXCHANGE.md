...
```bash
viz create my-exchange --from exchange --local
# edit my-exchange/content.js — and, for page-specific styling, its index.html
viz check my-exchange   # before you open a browser
```

`--from exchange` composes with `--local` / `--global` like any template.
To fork a working one instead, `viz create <new-slug> --from <viz-folder>`.
