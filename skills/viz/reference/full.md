...
| | |
|---|---|
| **create**, **templates** | a new viz — blank, or a copy of a template |
...
`mcp.ts` is that server, and its tools are generated from this same command tree — so every
verb here is reachable there, and neither surface can drift from the other. It ships as a
one-click `.mcpb` bundle with Bun inside; the install guide in the self-portrait has the
download. ChatGPT desktop needs none of this: it is the Codex app, so the `npx skills`
install runs in full mode there (Work mode or the Codex side — Chat mode does not load local
skills).
...
A viz is identified by **its path relative to your home directory**, which is also its URL — e.g. central `~/.agents/state/viz/foo` serves at `/.agents/state/viz/foo/`, and repo-local `~/Code/app/viz-pages/bar` serves at `/Code/app/viz-pages/bar/`. Real filesystem paths are globally unique, so two repos can both have a `dashboard` viz and never collide.
...
The same mechanism **adopts static HTML you didn't scaffold** — drop a folder with an
`index.html` into the library, rescan, and it serves with hot reload, the review layer
and publishing, its own runtime untouched. Recipe and caveats: `reference/ops.md`.
...
Resolution order is `$VIZ_PAGES_DIR` → `~/.agents/state/viz` (the default, and where this setup keeps any state a skill owns) → `~/.viz-pages` → `~/.claude/viz-pages`. The two older paths are still checked, in that order, so a machine whose library has not been moved yet keeps loading it. Set `VIZ_PAGES_DIR` to relocate.

## Step 1: Bootstrap

Argument: `/viz [name] [--local [dir]] [--global] [--from <template>] [--hero] [--quick]` — optional human-readable slug (e.g. `import-graph`, `bar-chart-population`). If absent, pick one based on what's being visualized. Use kebab-case.

**Templates come first.** When the viz has a known shape, a template replaces the blank starter. **Run `viz templates <kind>` before creating**, and follow the rule in `reference/templates.md`: none fits → blank starter; one → offer it; **more than one → ask the user which**.

| built-in template | use it when the viz is… |
|---|---|
...
More (a brand deck, say) can live in the central library or any repo's `viz-pages/`; `viz templates` lists them all. `--hero` is an add-on, not a template: a normal page that needs a share card *different* from itself.

**Using a template, making one, or editing a page that carries `viz:template` / `viz:page-kind`? Read `reference/templates.md` first** — what each one gives you, the safe zones, and the rules that keep the kind readable. Skip it for a plain page.
...
**On a fresh install these return nothing, and that is fine** — the library starts empty and fills up as you work. Don't stall hunting for prior art that can't exist yet: take the template that fits (above), or start from the blank starter and the kit. The bundled templates and `viz-self-portrait` are always present, and the self-portrait is a real worked example if you want to read one.
...
> **Bootstrap already created `index.html`** (pre-stamped with posture metas) **and printed it to stdout** — so you can `Edit` it straight away without reading it back. Keep the `viz:*` meta lines; edit everything else freely. (A page over 8KB — a deck — isn't dumped; read that one, since its comments are the authoring guide.)
...
### Hand off the URL before you polish it

`viz create` opens a browser tab, so the reader has the page from the moment it exists. Say the
URL in prose as soon as it renders something real — not at the end of your turn.

Post `ready` when you hand back. `building` is inferred from every file write, so this is the
only signal you send by hand:

```bash
curl -sX POST --data ready "http://127.0.0.1:5180/<viz-id>/_status"
```

The page shows a corner badge — amber `◐ building`, green `● ready` which fades after a few
seconds. It's injected at serve time like the reload script, so it never touches `index.html`
and cannot reach a published build.

### Verify before you commit

The viz isn't done when the code is written — it's done when it renders correctly. Past vizzes shipped layout bugs (overflowing labels, disconnected arrows) and silent JS errors (a typo'd function, a 404'd CDN import → blank page, no signal) that then took several follow-up commits to clean up, because nothing was checked before committing. **Always run the render check before committing** — don't eyeball it and hope. Run it *after* you've handed off the URL, not before — it can take minutes, and that should land while the reader is already looking at the page.
...
It drives headless Chrome once, writes `latest.png` / `console.txt` / `network.txt` / `dom.html` under `$SKILL_DIR/.verify/`, and prints two reports to stdout: a **layout audit** (clipping, some SVG text overflow, a mark census — each check is narrower than its name, so read the scope table in `reference/verify.md` before treating a clean run as "it looks fine") and an informational **visual-density** line. Neither blocks a commit.
...
```bash
viz move   <viz-folder> <dest-folder>          [--break-links]
viz delete <viz-folder>                        [--break-links]
viz update <viz-folder> [--posture …] [--listed …] [--triaged …]
                                               [--title …] [--description …] [--tags a,b,c]
                                               [--linked-from <text>] [--unlinked-from <text>]
...
Flags, failure modes, and the gotchas that bite (a move 404s the old URL, and a linked viz refuses to move without `--break-links`; `mirror add` and `vendor` both require `--access`): `reference/manage.md`.
