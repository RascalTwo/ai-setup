...
A viz does not have to come from `viz create`. **Any folder with an `index.html`
becomes one** — a reveal.js deck from another skill, an exported report, a prototype
someone handed you. It keeps its own markup, CSS and runtime; it gains hot reload, the
anchored review layer, publishing and git history.

```bash
cp -R <their-folder>/. "$VIZ/<slug>/"          # or <repo>/viz-pages/<slug>/ for repo-local
curl -sX POST http://127.0.0.1:5180/_rescan    # REQUIRED - see below
```

Then open `/<path-relative-to-$HOME>/`.

- **The rescan is not optional, including centrally.** Measured: a folder dropped into
  `$VIZ` and fetched without one returns **404**; after the rescan, 200. The Rescan
  button in the self-portrait does the same thing.
- **Relative subpaths just work.** Verified against a deck carrying a vendored
  `reveal.min.js`, webfont `.woff2`s and ~100 PNGs — every one served 200.
- **A foreign runtime coexists with the injected scripts.** reveal.js 5 plus the
  hot-reload and comment-layer injections: 0 console errors.
- **To publish it, add the `viz:*` metas** (`posture`, `listed`) to its `index.html`.
  An undeclared posture makes the whole publish run refuse.
- **It does not get the kit** unless its own HTML links it, and that is usually correct.
  Don't retrofit kit tokens into somebody else's design system.
...
- **Don't edit the adopted source to suit viz.** If it came from another skill or repo,
  that file has an owner. Change it in the host repo or send it upstream.
