...
```bash
viz preview <container> [--port <n>] [--open]      # SEE what would publish, served locally
viz publish <container> [--out <dir>] [--base-url <url>]   # build a whole container
viz export <vizDir> [--out <dir>]                  # one viz (dev/test primitive; no index)
viz rotate <vizDir>                                # revoke + re-mint a private link
...
```bash
viz preview <container> [--port <n>] [--open]
```

- **Side-effect-free by design.** It is the build-and-STOP core *without* the outbound steps: it **never pushes mirrors or vendored copies** into other containers and **never deploys**. Previewing a source container does not touch its mirror or vendor sinks. Nothing is committed.
...
- **Card text** comes from each viz's own `<head>`: title from `<meta name="viz:title">` (else `<title>`), blurb from `<meta name="viz:description">` (else `<meta name="description">`), and optional eyebrow tags from one or more `<meta name="viz:tag">` elements — repeat the element to attach several tags (each renders as its own chip).
...
### Spoiler cards — blur a card until it's clicked

```html
<meta name="viz:spoiler" content="true">
```

Opt-in, off by default. Marks a viz as spoilery so the lobby **blurs its hero image
and its blurb** until the viewer clicks that card once to reveal. The title stays
visible, and it re-blurs on reload.

The point is that it lets you author an honest, spoiler-full hero and
`viz:description` instead of a watered-down one — the lobby does the hiding, so the
card itself doesn't have to be coy.

**It only guards the lobby, not the OG unfurl.** A link shared into Slack/Discord
still previews the full hero — that surface belongs to the poster, so strip the
spoiler there yourself if it matters.
...
- **Image** (`og:image`) — optional, **static**, **1200×630** (1.91:1), **≤~300 KB**.
  **1200×630 is the one ratio verified to unfurl cleanly on Slack, X, Teams, WebEx
  and Discord.** Those platforms center-crop anything taller into a wide band, so a
  square, 16:9 or vertical card loses its top and bottom on a link post — which is
  why the `--poster` scaffold is fixed at this size and bakes in a ~1080×565 safe
  zone rather than leaving the aspect up to the author. Animated GIFs animate in cards on Discord *only* (everywhere else shows frame 1), so don't bother — a crisp still is the whole game. `og:url`/`og:image` are emitted only with a real `--base-url` (absolute URLs are required; crawlers reject relative and `data:` URIs).
...
```bash
viz verify <id> --og     # 1200×630 shot → <vizdir>/og.auto.png
```
...
## Refreshing vendored copies — `--push-vendors`

A **vendored** copy is this container's viz living as *source* in another repo, so that
repo can run and publish it on its own (contrast a mirror, which lands a built artifact —
see `reference/manage.md`). Edges are declared at the **origin**, in the same
`mirrors.json`, under a `vendors` array (ADR 0010).

```bash
viz publish <origin-container> --push-vendors
```

**It is opt-in on purpose.** Unlike a mirror push this writes recursive *source trees* —
and prunes with `rm -rf` — into another repo's working tree, so it must never be a side
effect of "I ran a build". Two gates:

- **no `--push-vendors` → nothing is written.** The run just reports how many declared
  copies it left alone.
- **`--out` set → still skipped**, because that run is a throwaway dist someone is
  inspecting, not a publish. `preview` never pushes either.

What a push does, per declared sink:

1. Copies each declared viz dir verbatim, stripping local-only files (`comments.json`,
   the markers, `.runtime/`, `.verify/`) so nothing machine-local rides into someone
   else's repo.
2. Writes the `.vendored.json` receipt — `{ origin, access, vendoredAt }`. `origin` is a
   `$HOME`-relative id, **never an absolute path**, since the receipt is committed in the
   sink.
3. **Origin-scoped prune** — removes only sink dirs whose receipt says they came from
   *this* container and are no longer declared. Never touches the sink's own vizzes or
   another origin's copies.

Two refusals worth knowing:

- **`access` must equal the origin's `viz:posture`.** The copy is byte-identical, so
  posture can't be re-framed the way a mirror's can — a mismatch fails that edge rather
  than rewriting the copy (rewriting would fork the keystore and mint a dead magic link).
- **Any failed edge cancels the prune for that sink.** A rename moves a declaration to a
  new slug, which makes the old copy prunable — if the replacement write also failed,
  pruning would delete the sink's only copy. Stale beats absent.

Drift is caught separately by `manage.ts vendor-check`, which byte-compares each copy
against its **live origin** (so it sees the origin moving forward, not just copy edits).

## Deploying — per-container `deploy.sh`, all at once with `deploy-all.ts`

`build.ts` stops at a dist dir. Getting those bytes onto a host is the container's own
`deploy.sh`, which lives in the container rather than the skill because only the container
knows its host, its remote, and its base URL. Presence of an executable `deploy.sh` is
therefore also the signal that a container is meant to be published at all.

```bash
bash <container>/deploy.sh       # one container (DRY_RUN=1 builds without pushing)
viz deploy-all   # every container set up for it
```

`deploy-all.ts` exists so "publish everything" isn't a hand-rolled loop. Two guards let it
be run broadly, and both are deliberate:

- **Opt-in by presence.** A container with no `deploy.sh` is skipped silently. That is how
  client repos, scratch containers, and the central library stay off the internet — there
  is no separate denylist, so never add a `deploy.sh` to a container you don't want shipped.
- **Committed state only.** A container whose `viz-pages` subtree is dirty is skipped rather
  than deployed, so what ships always matches what's in git. Commit first, then deploy.

Where a container wraps its deploy in a `deploy-live.sh` preamble, that wrapper runs instead
of `deploy.sh` (it is expected to `exec` the plain one at the end).

A `deploy.sh` need not target Pages at all — one of my private containers deploys behind
an SSO gate instead. Read the container's own before assuming what it does.
...
- **Deploying stays human-confirmed** — run `deploy.sh` / `deploy-all.ts` when the user says so, never on your own initiative.
