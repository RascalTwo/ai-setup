# /viz — fallback mode (no Bun, no server)

You are here because the mode check in `SKILL.md` resolved to **fallback** — you can't run the
...
Fallback lowers the *plumbing*, never the bar.

**What fallback is:** you write **one self-contained `.html` file** and hand it to the user. No
...
opened in a real browser. That's the one full-mode luxury fallback keeps — but reach for one
...
- **`kit/viz.js` — inline it only if you actually call it.** Most fallback pages need `stepper()`
...
There is deliberately no separate fallback copy of these files. Two copies of the same design
tokens drift, and then fallback pages stop looking like real vizzes.

## Templates still work

The built-in templates are folders under the skill's `viz-pages/`: `deck/`, `poster/`,
`poster-dive/` and `exchange/` (whose `content.js` is the part you edit), plus
`hero-template.html` in the skill directory. If the content has one of those shapes — a
presentation, a share card, an actors-and-packets exchange — read the template and adapt
it inline rather than rebuilding the chrome. Drop any `/_kit/` links and inline as above.
`kit/EXCHANGE.md` is the authoring guide for the exchange runtime. A deck inlines
`kit/deck.css` and `kit/deck.js` the same way; keep the two `import` lines at the top of
`deck.js` — they load reveal.js from the CDN, and the deck runs on it.
