...
**Then, after reading the rest of this file:** full mode → `reference/full.md`. Fallback mode →
`reference/fallback.md`. Those are not optional appendices — they are the other half of the
...
Hand-rolled SVG (`<rect>`, `<line>`, `<path>`, `<text>`) is often the cleanest answer. Reach for D3 for layout math, three.js for 3D, Canvas for high element counts. What else is reachable with no install — webfonts, vendor logos, syntax highlighting, and the few libraries that have earned their place — is in `reference/cdn.md`.
...
A **template** is a finished starting page for a known shape — a deck, a share card, an
exchange, or a brand variant someone built. Starting from one beats redrawing it from
memory, which is how a template ends up rebuilt partially.

**Before `viz create`, run `viz templates <kind>`** (or `viz templates` for every kind):

- **none fits** → the blank starter and the kit (below);
...
Details, the built-ins, and how to make one: `reference/templates.md`. In fallback mode
there is no CLI; the built-in templates are in the skill's `viz-pages/`
...
- **Fallback mode** → read **`reference/fallback.md`** now. How to inline the kit and hand the user
