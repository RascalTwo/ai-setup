...
for (const c of allContainers()) {
  ...
  // A repo may wrap deploy.sh in a deploy-live.sh preamble that has to run first
  // (explorables refreshes its mirrored-in vizzes, then execs deploy.sh). The
  // wrapper is a strict superset, so prefer it wherever one exists.
  const live = path.join(c, "deploy-live.sh");
  const entry = existsSync(live) ? live : sh;
  if (!(statSync(entry).mode & 0o111)) {
    results.push({ c, status: `⚠️  ${path.basename(entry)} not executable (chmod +x it)` });
    continue;
  }
  ...
  console.log(`\n🚀 ${c}${entry === sh ? "" : ` (via ${path.basename(entry)})`}`);
  const run = Bun.spawnSync(["bash", entry], { cwd: c, stdout: "inherit", stderr: "inherit" });
