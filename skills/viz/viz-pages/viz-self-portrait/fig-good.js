...
/* ---- stop 7 · every built-in template ------------------------------------
 ...
 * page around. Now it is one scrubbed walk: one template per step, each with
 * a working miniature of what that template actually produces. --hero rides
 * along as the last step: an add-on to any of them, not a shape of its own.
 */
const TEMPLATES = [
  { flag: '--from deck', name: 'Slide deck', tone: 'var(--accent)', stamp: 'a copy carries <code>viz:page-kind = deck</code>',
  ...
  { flag: '--from poster', name: 'Poster', tone: 'var(--c4)', stamp: 'a copy carries <code>viz:page-kind = poster</code>',
  ...
  { flag: '--from poster-dive', name: 'Poster + dive', tone: 'var(--c4)', stamp: 'a copy carries <code>viz:page-kind = poster-dive</code>',
    one: 'That card on top, a full deep-dive page underneath.',
    body: 'The card still clips to the share image, and everything below it is the long read. <b>This is the most-used template in the library by a wide margin</b> — one link that both previews well and rewards a scroll.',
  ...
  { flag: '--from exchange', name: 'Exchange', tone: 'var(--c5)', stamp: 'a copy carries <code>viz:page-kind = exchange</code>',
  ...
  { flag: '--hero', name: 'Hero card', tone: 'var(--warn)', stamp: 'adds <code>hero.html</code> — no kind of its own',
    one: 'A share card that is not the page itself.',
    body: 'Adds a <code>hero.html</code> beside the viz and leaves <code>index.html</code> alone. Built on the kit\'s OG stylesheet, so it re-themes with the viz instead of drifting into a parallel palette. It is the only one here that is <b>not</b> a template — it is an add-on to any page, not a shape.',
...
export const templates = {
  ...
  render(el, { onStep }) {
    el.innerHTML = `
      <p class="fig-what">Every built-in template, one per step, with a working miniature of what each
         one actually produces. <code>viz templates</code> lists them — and any others installed, like a
         brand deck — and <code>--from &lt;name&gt;</code> starts from one. That is the whole interface.</p>
      <div class="tmpl-stage">
        <div class="tmpl-copy">
          <div class="tmpl-h"><code id="tmpl-flag"></code><b id="tmpl-name"></b></div>
          <p class="tmpl-one" id="tmpl-one"></p>
          <p class="tmpl-body" id="tmpl-body"></p>
          <p class="tmpl-when" id="tmpl-when"></p>
          <p class="tmpl-stamp" id="tmpl-stamp"></p>
        </div>
        <div class="tmpl-demo" id="tmpl-demo"></div>
      </div>
      <p class="fig-foot">No template at all gives you a plain page, which is still the most common thing
         by far — <b>166 of 235</b> vizzes here. A template is just a viz with a
         <code>viz:template</code> meta, so any viz you already have can become one, and
         <code>--from &lt;path&gt;</code> forks any viz at all. A copy keeps the kind — a copy of a deck
         is still a deck — but is never a template itself, and always resets to <b>local/unlisted</b>,
         because posture is a trust decision and is never inherited.</p>`;

    onStep(i => {
      const s = TEMPLATES[i];
      el.style.setProperty('--tone', s.tone);
      el.querySelector('#tmpl-flag').textContent = s.flag;
      el.querySelector('#tmpl-name').textContent = s.name;
      el.querySelector('#tmpl-one').textContent = s.one;
      el.querySelector('#tmpl-body').innerHTML = s.body;
      el.querySelector('#tmpl-when').innerHTML = `<b>Reach for it when:</b> ${s.when}`;
      el.querySelector('#tmpl-stamp').innerHTML = s.stamp;
      el.querySelector('#tmpl-demo').innerHTML = s.demo;
    });
  },
};
      ...
      <div class="lib-two">
        ...
        <div class="lib-card">
          ...
          <p>That is the <a href="#dashboard" id="lib-dash">Dashboard</a> on this page: every viz on
             the machine, filterable by posture, listing, kind, whether it has a backend, and which
      ...
      <div class="pub-two">
        <div class="pub-card">
          ...
          <p>Cards carry each viz's title, blurb and tags — read from its own <code>&lt;head&gt;</code>,
             ...
             tags and kind. You can drop a <code>_preamble.html</code> at the container root to put
...
const FATE = {
  'local|unlisted':   { ship: 'no',  txt: 'Never published. The default — a new viz is invisible on both axes until you decide otherwise.', tone: 'var(--muted)' },
