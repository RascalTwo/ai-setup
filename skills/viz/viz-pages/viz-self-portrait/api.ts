...
import { approvalOf } from "../../lib/publish/approval.ts";
import { readdirSync, statSync, existsSync, readFileSync, renameSync } from "node:fs";
...
function resolveVizRoot(): string {
  ...
  // Order must match reference/full.md and discovery.ts exactly. `.agents/state/viz`
  // (ADR 0012) was MISSING here: this mirror kept only the two older paths, so the
  // dashboard resolved central to a leftover empty ~/.claude/viz-pages, read its stale
  // .discovered.json, and showed zero central vizzes while reporting a stale container
  // count. The header above promises these stay in lockstep — they had not since ADR 0012.
  const current = path.join(HOME, ".agents", "state", "viz");
...
const RANKINGS = path.join(CENTRAL, "rankings.json");
const blankList = (name: string) => ({ name, log: [], benched: [], priority: [], updated: null });
...
async function manage(args: string[]): Promise<{ ok: boolean; out: string; err: string }> {
  ...
  const proc = Bun.spawn([process.execPath, managePath, ...args], { stdout: "pipe", stderr: "pipe" });
...
// Lift the lobby key out of a preview build's stdout. build.ts already prints all of it for a
...
const SCAFFOLDS = ["poster", "poster-dive", "deck", "exchange"] as const;
...
function readAxes(dir: string): Axes {
  try {
    ...
    return {
      ...
      // Which built-in scaffold generated the page; "" = the blank starter (a plain
      // page, with or without a hero.html). A closed enum, unlike the open viz:tag set —
      // so an unrecognised value is normalised to "" rather than trusted through.
      scaffold: SCAFFOLDS.find((k) => k === grabMeta(html, "viz:scaffold").toLowerCase()) ?? "",
    };
  } catch {
    return { posture: "untagged", listed: true, approval: "never" as const, title: "", description: "", tags: [], scaffold: "", uid: "" };
  }
}
...
export default {
  ...
  // ---- ranking (ADR 0014 + kit/pairwise.js) ----------------------------------
  // Items are DERIVED from the live corpus on every read; only the decision log is
  // stored. That split is the point: a viz that moves, is retitled, or is created
  // afterwards still matches its answers, because identity is viz:uid and nothing
  // here remembers a path. A viz with no uid (a mirror, a vendored copy, or anything
  // not yet backfilled) is excluded rather than given a fragile identity.
  //
  // Persisted in CENTRAL, not localStorage: ranking ~260 vizzes is ~1,900 comparisons
  // gathered over weeks, and a browser-data clear must not be able to delete that.
  // ---- ranking (ADR 0014 + kit/pairwise.js) ----------------------------------
  // Items are DERIVED from the live corpus on every read; only the decision log is
  // stored. That split is the point: a viz that moves, is retitled, or is created
  // afterwards still matches its answers, because identity is viz:uid and nothing
  // here remembers a path. Copies (mirror/vendored) are skipped — they are the same
  // viz as their origin and would otherwise appear on both sides of a comparison.
  //
  // MANY NAMED LISTS, not one. "Best" is only one thing you might rank on; the same
  // corpus can be ordered by "most reusable", "prettiest", "best for a client demo".
  // Each dimension keeps its own independent log, so answering one never disturbs
  // another and you can add a dimension later without losing anything.
  //
  ...
  "/ranking": async (req: Request) => {
    ...
    for (const s of listSlugs()) {
      ...
      const a = readAxes(s.dir);
      if (!a.uid) { noUid++; continue; }
      const og = ["og.png", "og.jpg", "og.auto.png"].find((f) => existsSync(path.join(s.dir, f)));
      items.push({
        title: a.title || s.name,
        url: "viz://" + a.uid,
        key: "viz://" + a.uid,          // EXPLICIT identity: not title-derived, so a retitle
                                        // does not orphan this viz's answers
        desc: a.description,
        media: og ? ["/" + s.id + "/" + og] : [],
        tags: [s.isCentral ? "central" : path.basename(path.dirname(s.container)), a.posture],
        vizId: s.id,
        approval: a.approval,
      });
    }
  ...
  "/slugs": async () => {
    const natives = listSlugs().map((s) => {
      ...
      return {
        ...
        tags: axes.tags,
        scaffold: axes.scaffold, // "" = plain page; else poster | poster-dive | deck | exchange
  ...
  "/update": async (req: Request) => {
    ...
    for (const axis of ["posture", "listed", "approved"]) {
  ...
  "/preview": async (req: Request) => {
    ...
    const proc = Bun.spawn([process.execPath, buildPath, "preview", container], { stdout: "pipe", stderr: "pipe" });
        ...
        try {
          while (true) {
            ...
            if (!found) {
              ...
              // Anchored to the "👀 Preview" banner, which build.ts prints LAST — a bare
  ...
  // POST {container}. Runs <container>/base-url.sh and returns its URL. Deliberately NOT
  // called on scope selection — only on an explicit click — because this executes a script
  // out of whichever repo is selected, and the answer feeds share links that carry the lobby
  // key. Needs no build, so it costs a shell-out rather than a 180s publish.
  //
  // Fails CLOSED: exit 0 is not enough, stdout must actually look like a URL. A script that
  // prints a diagnostic where a URL belongs would otherwise render a link to nowhere.
  "/base-url": async (req: Request) => {
    ...
    if (!existsSync(script)) return Response.json({ ok: false, absent: true, err: "no base-url.sh — this container declares no deployed URL" });

    let proc: ReturnType<typeof Bun.spawn>;
    try {
      proc = Bun.spawn([script], { cwd: container, stdout: "pipe", stderr: "pipe" });
    } catch {
      return Response.json({ ok: false, err: "base-url.sh is not executable — chmod +x it" });
    }
    // A derivation may hit the network (one of mine queries CloudFormation), so cap it rather than
    // letting one unreachable API wedge the request.
    ...
    if (code !== 0) return Response.json({ ok: false, err: rawErr.trim() || `base-url.sh exited ${code} — no deployed URL` });
    if (!/^https?:\/\//i.test(out)) return Response.json({ ok: false, err: `base-url.sh exited 0 but printed no URL: ${out.slice(0, 120) || "<empty>"}` });
    return Response.json({ ok: true, url: out.replace(/\/+$/, "") });
  },
