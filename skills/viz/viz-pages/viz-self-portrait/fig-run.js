...
// The Gem's share link. publish-gem.py updates this same Gem in place, so it never changes.
const GEM_URL = 'https://gemini.google.com/gem/1Dcv4NlDKSyh8pTc3MyNgr01JUM1ChpSs?usp=sharing';
    ...
    const WAYS = [
      {
        id: 'agents',
        label: 'Coding agent',
        sub: 'Claude Code, Codex, ChatGPT desktop, Cursor…',
        mode: 'full',
        ...
        body: `
          ...
          <p class="fig-what"><b>ChatGPT desktop</b> is the Codex app, so this covers it too — on every
             plan, Free included. Use <b>Work</b> mode or the Codex side and type <code>@viz</code>;
             <b>Chat</b> mode does not see local skills.</p>`,
        cmds: [
      ...
      },
      {
        ...
        sub: 'the Claude chat app on Mac or Windows',
        mode: 'full',
        modeNote: 'Same as a coding agent — the MCP server runs on your real machine, outside the sandbox.',
        body: `
          <p class="fig-what">Claude Desktop runs skills in a sandbox with no shell, no Bun and no reachable
             <code>127.0.0.1</code>. It also runs <b>local MCP servers as ordinary processes on your
             machine</b>, outside that sandbox — so the bundle below, which is <code>mcp.ts</code> with Bun
             inside, hands it the real thing in one click.</p>`,
        download: true,
        cmds: [],
      },
      {
        ...
        body: `
          ...
          <p class="fig-what">You do not choose this — the skill works out that it is in a sandbox and
             ...
             dropping down.</p>
          <p class="fig-what">To get the skill there, zip the <code>skills/viz/</code> folder and upload it
             as a custom skill — claude.ai (web and mobile) and ChatGPT both take the same zip. claude.ai needs Pro or above; ChatGPT needs a Business,
             Enterprise, Healthcare or Edu workspace — <b>Free, Plus and Pro cannot upload skills</b>.</p>`,
        cmds: [],
      },
      {
        id: 'gemini',
        label: 'Gemini',
        sub: 'the Gemini app, web or mobile',
        mode: 'fallback',
        modeNote: 'One self-contained HTML page, built in Canvas. No server, no git, no publishing — same design bar.',
        body: `
          <div class="dl">
            <p class="dl-lead"><b>One click.</b> Open the Gem while signed in to Google, and ask it to
               visualize something. Nothing to install or upload.</p>
            <div class="dl-grid">
              <a class="dl-card" data-viz-id="gem-link" data-label="open the viz Gem in Gemini"
                 href="${GEM_URL}" target="_blank" rel="noopener">
                <b>Open the viz Gem</b>
                <i>gemini.google.com · Canvas is its default tool</i>
              </a>
            </div>
          </div>
          <p class="fig-what">A Gem runs no code of its own, so this is fallback mode: the whole skill — the
             method, the kit, the built-in templates — lives in the Gem's instructions, and every
             answer is one HTML page rendered in Canvas.</p>
          <p class="fig-what"><b>It keeps itself current.</b> Every viz release updates this same Gem in
             place, so the link never changes and there is nothing to re-download — unlike the
             <code>.mcpb</code>, which has no update mechanism.</p>
          <p class="fig-what">A work or school account may be blocked from opening Gems shared from
             outside its organization; that is the admin's setting, not the Gem's.</p>`,
        cmds: [['Gem link', 'share it as-is — the link survives every update', GEM_URL]],
      },
      {
        id: 'dev',
        label: 'Work on it',
