...
// URLs — so the page's own `import ... from "/_kit/<name>.js"` needs no rewriting.
//
...
const KIT_JS_REF = /["'`]\/_kit\/([A-Za-z0-9._-]+\.js)["'`]/g;
function kitImportMap(html: string): string {
  const imports: Record<string, string> = {};
  const queue = ["viz.js", ...[...html.matchAll(KIT_JS_REF)].map((m) => m[1])];
  ...
  while (queue.length) {
    ...
    imports[`/_kit/${name}`] = "data:text/javascript;base64," + Buffer.from(js, "utf8").toString("base64");
    for (const m of js.matchAll(KIT_JS_REF)) queue.push(m[1]);
  }
...
export function buildSelfContained(vizDir: string, overrides?: HeadOverrides): BuildResult {
  ...
  html = html.replace(
    /<script([^>]*)\ssrc=["'](\/_kit\/[A-Za-z0-9._-]+\.js)["']([^>]*)>\s*<\/script>/gi,
    (_m, before, spec, after) => `<script${before}${after}>import ${JSON.stringify(spec)};</script>`,
  );
