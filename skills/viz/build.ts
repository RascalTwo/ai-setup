...
import { rotateKey } from "./lib/publish/run.ts";
...
import { getOrCreate, type KeyEntry } from "./keystore.ts";
...
const { flags: buildFlags, pos: positional } = import.meta.main
  ? parseFlags(argv, {
      ...
      known: [...BUILD_VALUE_FLAGS, "no-index", "open", "lobby", "push-vendors", "no-deploy-notice", "json", "break-links"],
    })
  : { flags: {}, pos: [] as string[] };
...
if (cmd === "rotate") {
  // One implementation, so this entry point cannot skip the linked-viz guard (ADR 0016).
  await rotateKey(positional[1], { lobby: lobbyFlag, breakLinks: bool(buildFlags, "break-links") });
}
