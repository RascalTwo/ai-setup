...
for (let i = 0; i < argv.length; i++) {
  ...
  else if (a === "--push-vendors") pushVendorsFlag = true;
...
function readMirrors(container: string, nativeSlugs: Set<string>): { mirrors: MirrorTarget[]; vendors: VendorTarget[] } {
...
const VENDOR_RECEIPT = ".vendored.json";
...
const VENDOR_STRIP = ["comments.json", VENDOR_RECEIPT, MIRROR_SIDECAR, ".DS_Store", ".runtime", ".verify"];

async function pushVendors(container: string, vendors: VendorTarget[]): Promise<void> {
