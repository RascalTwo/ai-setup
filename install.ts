...
if (process.argv.includes("--externals")) {
  ...
  // Overlays can list their own (e.g. a private repo's private skills); merged, core first.
  const repos: Record<string, string[]> = {};
  for (const root of [REPO, ...overlays]) {
    const mf = join(root, "external-skills.json");
    if (existsSync(mf)) Object.assign(repos, JSON.parse(readFileSync(mf, "utf8")).repos ?? {});
  }
...
console.log("== run-tab (prefix+c → new herdr tab running a typed command) ==");
{
  // Same as claude-tab, plus unbinding herdr's own new_tab from prefix+c. That goes
  // inside an existing [keys] table if there is one: a second [keys] header is invalid TOML.
  if (haveHerdr && existsSync(HERDR_CFG)) {
    const cfg = readFileSync(HERDR_CFG, "utf8");
    if (!/^\s*new_tab\s*=/m.test(cfg))
      writeFileSync(HERDR_CFG, /^\[keys\]\s*$/m.test(cfg)
        ? cfg.replace(/^\[keys\]\s*$/m, '[keys]\nnew_tab = ""')
        : cfg + '\n[keys]\nnew_tab = ""\n');
  }
  const keybind = join(REPO, "tools", "run-tab", "herdr-keybind.toml");
  if (haveHerdr && existsSync(keybind))
    appendTomlIfAbsent(HERDR_CFG, readFileSync(keybind, "utf8").replaceAll("~/", `${HOME}/`),
                       /run-tab/, "[[keys.command]] prefix+c / prefix+shift+c");
}
