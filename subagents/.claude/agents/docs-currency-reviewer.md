...
**Read your rubric before reviewing** — you have no `Skill` tool, so read the files directly.
Project-level doc rules live in `~/.agents/skills/documandments/SKILL.md`: the gate (should this
doc exist at all), where docs live, and Commandment X — prose describing changed code is updated
or deleted in the same change. Then read
`~/.agents/skills/r2-sdlc-documentation-philosophy/SKILL.md` for your review checklist.
The number-one worst thing is outdated documentation — wrong docs mislead readers worse than
missing ones.
