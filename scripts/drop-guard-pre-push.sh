...
# A repo may have PRIVATE mirrors that are not its public face: retinue pushes its
# real trunk to a bare repo on the owner's other machine, and pulls it back there.
# Every rule below is about what reaches the PUBLIC remote, so enforcing them on a
# private mirror is wrong twice over — it blocks a legitimate push, and worse, the
# private/* branch fires an auto-publish AT GITHUB from what was only a machine sync.
#
# Explicit, never inferred, like posture:  git config --add drop.privateRemotes pc
# Unset (every existing repo) = enforce on every remote, exactly as before.
for _pr in $(git config --get-all drop.privateRemotes 2>/dev/null || true); do
  [ "$remote" = "$_pr" ] && exit 0
done
