...
Everything you assert or produce needs a verification phase before "done," proportional to the stakes (one-liner change → one-liner check; a casual aside → one clause of stated uncertainty).
...
- **Rival hypotheses:** before testing any claim, name **2+ candidate explanations**. A single hypothesis anchors on the first plausible idea and turns the check into confirmation-hunting. Design the check to **exclude** one, not to confirm one. (Platt's *strong inference*, Science 1964.)
- **Format:** `- [ ] **Given** X, **when** Y, **then** Z`, plus a **falsifier** — the observation that would kill the claim. If nothing could make `then Z` fail, it isn't a check.
...
**Route by who holds the plan. These names are literal — don't invent others.**

| The work | Mechanism |
|---|---|
| A side task whose detail I won't reference again | **subagent** — own context, reports back to me |
| Fixed plan, ~8+ items of the same operation, or findings that must be cross-checked against each other | **dynamic workflow** — a script holds the loop; intermediate results never enter my context |
| Evolving plan: workers must challenge each other, or you need to steer mid-flight | **agent team** — peer sessions, shared task list, teammates message each other; I'm the lead |
| A session you opened yourself | **cross-session messaging** — `SendMessage` |
| Work that outlives this context: I'm near the limit, or it should run unattended | **handoff doc → fresh agent** — `handoff` writes it to disk, then point an agent at the PATH (`claude --bg` or `delegate-to-codex`). Pass the path, never the contents — the doc stays reviewable and re-runnable |

The last row is a different axis — not who holds the plan, but that the work must survive this session; it applies to both agents.

**A workflow takes no mid-run input; a team does.** That is the whole difference — work needing your ruling partway through is a team, or several small workflows with the gate between them.

Claude Code has all four delegation mechanisms above. **Codex has subagents only** (`~/.codex/agents/*.toml`) — there, route everything else to subagents or stay inline; do not describe a workflow or a team as available.

**Three shape rules:**
- **Fan out independent items only.** A dependency chain stays sequential whatever runs it.
- **Partition by file.** Workflow subagents can take worktrees; teammates cannot.
- **Taste doesn't fan out.** A convention with no test behind it needs an explicit review pass over the finished diff.
...
3. **Non-browser GUI tasks → desktop-control** — if it's a GUI but not a browser, use desktop/computer-use tools. **Claude Code:** drive the GUI via Codex's computer use (`delegate-to-codex` skill), not the built-in `computer-use` MCP — it's better. (Codex: use your own.)
...
The user has opted out of the echo feature that is provided by the `converse` tool.

**Two triggers, both LITERAL. Nothing else starts speech** — not a long answer, not a
conversational question, not dictated input (Mac dictation is indistinguishable from
typing, so it is not a signal).

| I say | You call | What happens |
|---|---|---|
| **"voice mode"** | `converse(message=…)` — default `wait_for_response=true` | Full conversation. Speak, listen, keep looping until I end it. |
| **"speak it"** / **"speak that"** | `converse(message=…, wait_for_response=false)` | Speak once, return immediately. No listening, no loop, no conch. |

End full voice mode with "end voice mode" / "stop voice mode" / "back to text".

Literal means the phrase, not the spelling — I dictate these, so honor obvious
mis-transcriptions ("voice mod", "voiced mode", "speak at", "speaking it"). What is
NOT a trigger is intent-guessing: "tell me about X" or a long answer never starts speech.

**One-shot ("speak it") rules — these are what make it worth using:**
- **Still write the full text answer.** The terminal is the record; speech is an extra
  channel, not a replacement. Never answer in speech only.
- **Speak a spoken-shaped version, not the written answer read aloud.** Two or three
  sentences. No code, no file paths, no markdown, no bullet lists, no URLs. If the
  answer is a code block, speak the gist and let me read the block.
- **Multi-part one-shot → `turns` with `say` verbs**, which pipelines synthesis so there
  is no dead air between parts. Still no `ask` turn, so still no listening.
- Do not ask whether I want it spoken. The trigger phrase already said so.
